...
def canli_uygulama_baglami_hazirla(gorev: bool = False) -> str | None:
    ...
    return (
        ...
        "1. Sayfa yüklenince `browser_snapshot` al; ekran adı/route, ana bölümler, başlıklar ve "
        "tablo kolonlarını kaydet. `browser_network_requests` ile açılış isteklerini (liste/lookup "
        "servisleri) yakala.\n"
        "2. TÜM tabları/segment kontrollerini SIRAYLA tıkla — snapshot'ta gördüğün her tab için: "
        "tıkla → snapshot → o tabın tablosu/formu/filtreleri → tetiklenen yeni istekler. Hiçbir tabı "
        "atlama; URL'deki `?tab=` yalnızca başlangıç noktasıdır.\n"
